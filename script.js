...
let wonCardsHolder = document.getElementById('wonCardsHolder');
let warCardsHolder = document.getElementById('warCardsHolder');
...
DistributeCards: function(player1Deck, player2Deck){
  for(let i = 0; i < this.shuffledDeck.length / 2; i++){
...
ReshuffleDeck: function(player) {
  while(player.wonDeck.length){
    let index = Math.floor(Math.randon() * player.wonDeck.length);
...
PlayGame = function(player1, player2){
  ...
  if(!player1.isReshuffling && 1player2.isReshuffling){
    player1.GetCurrentCard();
    player2.GetCurrentCard();
